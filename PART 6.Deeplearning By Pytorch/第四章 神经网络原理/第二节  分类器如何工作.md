...
<a id="markdown-二一个简单线性分类器工作原理" name="二一个简单线性分类器工作原理"></a>
#二、一个简单线性分类器工作原理
...
如果用数学语言来描述的话，就是对$\alpha$求偏导！高数告诉我们偏导数即为某点的斜率，下图中红色小球的起点，即$\alpha_{_1}$点的斜率用绿色的箭头表示，该线段在$\alpha=\alpha_{_1}$点处与曲线相切，箭头所指方向为小球滚动的方向，该方向亦为导数的反方向，仔细看看这个图，只要我们沿着导数相反的方向移动$\alpha$（对于$\beta$来说也是一样），就会找到极小值点了！
<div align=center>
<img src='http://qiniu.xdpie.com/2018-06-13-13-18-06.png'>
<p style='text-align:center'>图1.2.10</p>
</div>

对于上面的偏导$\frac{\partial \epsilon }{\partial \alpha }$我们也称为梯度，残差方程我们也称为损失函数，刚才讲的沿着导数的反方向向下移动寻找曲线极小值点的方法就叫梯度下降算法。在小球逐步往下移动的时候，每次移动的速率，我们用$\eta$表示，称作学习率，学习率的大小对于小球逼近极小值点有着至关重要的作用。较大的学习率会让小球错过极小值点，因为过大的学习率导致小球每次滚动的速率过大，在快要接近极小值点的时候却错过了极小值点。在实际应用中，这种情况会使我们的残差突然增大，来回摆动，梯度下降算法失效，所以我们通常会选取比较小的学习率，以避免逼近极小值点时来回震荡的情况。
<div align=center>
...
现在，我们来演示一下把多输入转化为矩阵，并线性分类器中进行计算的过程。如下图所示，已知四个输入信号$x_{1},x_{2},x_{3},x_{4}$，每一个输入信号与输入层的每一个节点都有权重相连，可以看到$x_{1}$与输入层的第一个节点权重为$ω_{1,1}$，$x_{2}$和输入层的第一个节点间的权重为$ω_{2,1}$，$x_{2}$和输入层的第二个节点间的权重为$ω_{3,2}$等等，以此类推......
<div align=center>
...
以上便是两个输入节点，两个输出的线性分类器的信号计算过程。这个过程相对来说比较简单，如果有四个输入节点的话，那么权重为4X4维矩阵。如果中间还有隐藏层，那么每一层之间的权重又不一样了。
...
至此，一组信号输入线性分类器及最后输出的过程就全部清晰的展示出来了。你也许会问还有一个重要的问题没有解决，就是权重如何优化呢，当然，这离不开梯度下降算法的功劳了，后面我们会用一个实际的例子并附上代码来训练一次。


<a id="markdown-四鸢尾花分类" name="四鸢尾花分类"></a>
#四、鸢尾花分类
...
好了，通过这一节我们逐步清晰了最简单的线性模型的工作原理，输入信号是怎么在网络中流动的，以及最后输出是些什么信号，这些细节无疑对于我们后续了解更复杂的网络打下坚实的基础。最后，请闭上眼睛，让神经网络在脑海里再漫游一次，让我们再编织一次网络模型，让数据再一次在模型中流动。睁开眼，带着愉悦的心情走向下一个里程。
