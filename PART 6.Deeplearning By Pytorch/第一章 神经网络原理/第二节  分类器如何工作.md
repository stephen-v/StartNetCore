...
可以看到，经过了10次迭代滞后，参数$\alpha$的值变成了1.00043，，这个和参数最优值1已经比较接近了，虽然这里给出的是一个非常简单的样例，但是已经完全可以说明梯度下降算法的工作原理了。同理，神经网络的优化过程也是可以类推的，这个过程可以分为两个阶段，第一阶段先通过前向传播算法计算得到预测值，即计算每一个样本的估计值$\alpha x_{_i}+\beta$，并将预测值和真实值做对比得出两者之间的差距，即得出残差方程。然后第二个阶段通过反向传播算法计算损失函数对每一个参数的梯度，再根梯度和学习率使用梯度下降算法更新每一个参数。
...
现在，我们来演示一下把多输入转化为矩阵，并在神经网络中进行传导计算的过程。下图展示了一个三层网络，已知四个输入$x_{1},x_{2},x_{3},x_{4}$，每一个输入信号与输入层的每一个节点都有权重相连，下图仅展示了一个输入信号和四个节点的权重关系，可以看到$x_{1}$与输入层的第一个节点权重为$ω_{1,1}$，下标$_{1,1}$表示第一个信号和第一个节点，同理$ω_{1,2}$表示第一个输入信号和第二个节点的权重，这样以此类推，第二个输入信号和输入层的第一个节点间的权重为$ω_{2,1}$......
<div align=center>
...
#四、鸢尾花分类
下面，我们收集到了一些鸢尾花数据集（在iris.xlsx文件中），部分数据如下表，在该数据集中有两种类别的鸢尾花各五十条，一共100条数据，数据集包括4个属性，分别是萼片长度，宽度，花瓣长度，宽度，其中在100条数据中，两类分别提取10条数据，共20条数据作为验证集，另外80条是训练集。采用最简单的阶跃函数做激活函数，再规定分类器输出1表示Iris-setosa(山鸢尾)品种，输出0表示Iris-Versicolour(变色鸢尾)。

在训练模型前，特别注意的是，我们需要把两类物品的特征数据（比如豆子的长宽、颜色数据，且是黄豆或蚕豆等真实值）分为训练数据和验证数据，然后把训练数据输入模型进行训练，最后通过验证数据来检验模型的准确率。
...
我们仅仅使用了萼片长度和宽度两种特征来描绘鸢尾花的散点图，从上图可以看到两个品种的鸢尾花呈现出明显的线性关系，也就是说，可以使用一个线性分类器把两类别数据划分开，这正是我们希望看到的情况。
那么从现在起，我们会时常使用Python来表达数学问题和训练模型过程，按照以上分析的结果首先定义一个线性网络，模型中初始化了输入、输出、权重、学习率和Sigmoid激活函数：
...
接着，我们把下载到的数据分成训练集和测试集分别放在两个csv文档中，既然数据量不是很大，这个工作需要我们手动来完成。

定义好了线性模型后，我们需要定义如何训练网络和测试网络。如下代码所示，把该线性模型保存在一个python文件中，为后续的训练和测试调用。
```python
import numpy
import scipy.special
...
```
紧接着开始训练网络，我们先把存放在csv文件上的数据导入一个list中，即使得输入信号为如下形式：
$$
\begin{bmatrix}
 [5.1&  3.5&1.4  &0.2 ]\\ 
 [4.9&  3&1.4  &0.2 ]\\ 
...
```
数据准备好后，可以开始训练模型了，首先我们根据原数据特性来设置输入节点个数（4个输入信号，分别是萼片长度、萼片宽度、花瓣长度、花瓣宽度）、输出节点个数（2个，用于最后判断是0还是1，1表示山鸢尾花，0表示变色鸢尾花）、学习率（0.01）、训练的迭代次数（1000次）、以及调用之前定义的模型。
```python
...
```
训练的迭代次数会影响模型的好坏，我们在本次训练中，由于训练数据仅有几十条，因此，迭代1000次完全足够了。在训练完模型后，模型保存了最优的权重值，在验证模型时，我们得使用这些权重值来做预测。验证后模型好坏标准我们可以用几个指标来衡量，准确率，召回率，F值等等。
...
了解到评估模型的几个性能指标后，我们开始正式验证该线性模型的表现力，下面代码中，省略了数据的导入，该部分与训练模型的时候一样。验证时，我们把模型最后输出节点上的两个概率值取最大概率值的索引坐标，然后后真实label做对比，如果和真实值一样，则统计为1，如果不相同，则统计为0，最后，用统计的值来计算准确率。
```python
...
这个结果对于仅用100多条数据来进行训练过的模型来说应该很不错了，而且我们从始至终都非常清晰我们的模型是如何工作的，信号是怎么在网络中流动的，这无疑对于我们后续了解更复杂的网络打下了坚实的基础。

最后，请闭上眼睛，让神经网络在脑海里再漫游一次，让我们带着愉悦的心情走向下一个里程。
