...
首先描绘一下我们的网络，如下图所示，我们采用了两层网络结构，中间省略了隐藏层，最后一层输出使用一个激活函数Sigmoid，在模型内部，输入层和输出层之间，我们分别用绿线代表输入信号和输出层第一个节点的连接，并且为输入信号赋予的权重用绿色表示；同理，红线表示输入信号和输出层中第二个节点的连接，并且为输入信号赋予的权重用红色表示，这样方便我们看清信号在网络中的流动状态。
<div align=center>
<img src='http://qiniu.xdpie.com/2018-06-08-10-59-40.png'>
<p style='text-align:center'>图1.2.14</p>
</div>

定义好了线性模型后，我们需要定义如何训练网络和测试网络。如下代码所示，把整个模型保存在一个python文件中，为后续的训练和测试调用。
```python
import numpy
import scipy.special
...
<div style='width:100px;hight:50px;margin-bottom:400px'></div>
