...
这样我们就把二维数据映射成了三位数据，对于高斯核的映射，会用到泰勒级数展开式，读者可以自行推导一下。
对于核函数我们就暂时介绍到这里。下一节我们开始运用tensorflow来进行实战，由于没有找到线性不可分的数据，我们的例子中就没有用到核函数来建模，因为我们只找到了一个线性可分的数据，所以下一节我们仅运用tensorflow来进行线性二分类的分类器构建。
## 4.6 实例