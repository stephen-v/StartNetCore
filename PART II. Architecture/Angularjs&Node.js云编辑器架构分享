...
**MenuConfiguration**:用于应用程序顶部菜单和右键菜单的配置,由于菜单功能复杂包括多级，而且多变因此采用配置结构。
...
代码管理采用自己搭建的gitlab,同时搭建了jenkins持续集成服务器,采用docker进行构建与部署，
当代码合并到release分支后自动集成打包部署。整个环节实现全自动化。

> 整个配置太多了就不讲解了，有需要可以留言，

## 五、打包

当前端需要发布时采用grunt对前端打包,如何打包请自行百度。

## 六、总结

整个项目工作量很多，我们团队6人，在规定时间内完成，持续集成自动化部署给我们节约了很多时间，
另外架构的设计让在开发过程中的需求微调赢的了时间。

同时希望大家可以参与讨论，提出问题或更好的解决方案和工具，让大家一起进步。
